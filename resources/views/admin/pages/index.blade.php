					...
					<div class="card">
						<div class="card-header">
							<div class="row mb-2">
								...
								<div class="col-sm-6">
									<a href="javascript:;" class="btn btn-primary float-right elevation-4" id="addButton"><i class="fas fa-plus"></i> Add Page</a>
								</div>
							</div>
						</div>
						<!-- /.card-header -->
	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="deleteModalLongTitle">Add Page</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			{{ Form::open(['route' => 'pages.store','role' => 'form', 'method' => 'post', 'id' => 'create-service']) }}
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group {{ $errors->has('title') ? ' is-invalid' : '' }}">
							...
							{{ Form::text('title', null, ['class' => 'form-control '.($errors->has('title') ? 'is-invalid':''), 'placeholder' => 'Page Title']) }}
							@if ($errors->has('title'))
...
<div class="modal-dialog modal-dialog-centered" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title" id="deleteModalLongTitle">Are you Sure You want Delete This page ??</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">&times;</span>
			</button>
		</div>
		<form action="{{ route('admin.pages.delete') }}" method="post" role="form" id="deleteForm" enctype="multipart/form-data">
			{{csrf_field()}}
	...
	$(document).ready(function () {
		$('#create-service').validate({
			...
			submitHandler: function (form) {
				addService('{{ route('pages.store') }}','create-service','services','addService');
		...
		submitHandler: function(form) {
			...
			$.ajax({
				...
				success: function (data) {
					$('#loading').hide();
					if(data.status==1){
						$( '.register-form-errors' ).html('<div class="alert alert-success">Pages Update Successfully</div>');
						setTimeout(function(){ 
							$("#editModal").modal('hide');
							window.location.reload();
						}, 3000);
